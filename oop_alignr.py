...
class ALIGNRResearch:
    ...
    def add_session(self, email: str, pre: str, ai: str,
                    pred: str = "", task: str = "general") -> dict:
    ...
    def compare_groups(self) -> dict:
        fb = [u.average_ras for u in self.users.values()
              if u.study_group == "feedback" and u.sessions]
        ct = [u.average_ras for u in self.users.values()
              if u.study_group == "control" and u.sessions]
        if len(fb) < 5 or len(ct) < 5:
            return {"feedback_n": len(fb), "control_n": len(ct),
        ...
        return {
            "feedback_n": len(fb), "feedback_avg": round(float(np.mean(fb)), 4),
            "control_n":  len(ct), "control_avg":  round(float(np.mean(ct)), 4),
            "t_stat": round(float(t), 4), "p_value": round(float(p), 4),
            "cohens_d": round(float(d), 4), "significant": bool(p < 0.05),
        }

    def export_summary(self, path="research_summary.json") -> dict:
        data = {
            "total_users": len(self.users),
            "feedback_users": sum(1 for u in self.users.values()
                                  if u.study_group == "feedback"),
            "control_users": sum(1 for u in self.users.values()
