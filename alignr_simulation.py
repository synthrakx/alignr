...
def run_simulation(
    n_feedback: int = 120, n_control: int = 80, sessions_per_user: int = 10
) -> ALIGNRResearch:
    """
    ...
    print(
        f"Building simulation: targeting {n_feedback} feedback + "
    ...
    while feedback_count < n_feedback or control_count < n_control:
        ...
        if (feedback_count + control_count) % 50 == 0 and (
            feedback_count + control_count
        ) > 0:
            print(
                f"  Progress: {feedback_count} feedback + "
                f"{control_count} control..."
            )

    print(
        f"Simulation complete: {feedback_count} feedback + "
...
def analyse(research: ALIGNRResearch) -> dict:
    ...
    if p is not None:
        conclusion = (
            f"Feedback group RAS {fb_avg:.3f} vs "
            f"Control {ct_avg:.3f} "
...
def build_session_matrix(research: ALIGNRResearch, metric: str = "ras"):
    ...
    for user in research.users.values():
        scores = [
            getattr(s, metric) for s in user.sessions if getattr(s, metric) is not None
        ]
        if len(scores) >= 10:
...
def figure1_ras_trajectory(
    research: ALIGNRResearch, path: str = "alignr_fig1_ras_trajectory.png"
):
    ...
    ax1.set_title(
        "Figure 1A: RAS Trajectory (Simulated)", color="#ffffff", fontweight="bold"
    )
    ...
    ax2.set_title(
        "Figure 1B: Final Session Comparison", color="#ffffff", fontweight="bold"
    )
    ax2.tick_params(colors="#7a7a9a")
    ax2.grid(True, alpha=0.1, axis="y")

    plt.suptitle(
        "ALIGNR Study — Figure 1 (SIMULATED)", color="#5a6a8a", fontsize=9, y=0.02
    )
    plt.tight_layout()
...
def figure2_cii_progression(
    research: ALIGNRResearch, path: str = "alignr_fig2_cii_progression.png"
):
    ...
    ax.set_title(
        "Figure 2: CII Progression Over 10 Sessions (Simulated)\n"
    ...
    plt.suptitle(
        "ALIGNR Study — Figure 2 (SIMULATED)", color="#5a6a8a", fontsize=9, y=0.02
    )
    plt.tight_layout()
    plt.savefig(path, dpi=150, bbox_inches="tight", facecolor="#0a0a0f")
    plt.close()
...
def figure3_scs_calibration(
    research: ALIGNRResearch, path: str = "alignr_fig3_scs_calibration.png"
):
    """
    ...
    titles = [
        "Figure 3A: Session 1 SCS Distribution",
    ...
    plt.suptitle(
        "Figure 3: Surprise Calibration Score Distribution (SIMULATED)\n"
...
def export_portfolio(
    research: ALIGNRResearch, stats: dict, path: str = "portfolio.json"
) -> dict:
    """
    Export clean summary. No raw text anywhere. All floats.
    Privacy proof: open portfolio.json → zero text values from user input.
    """
    feedback_n = sum(1 for u in research.users.values() if u.study_group == "feedback")
    control_n = sum(1 for u in research.users.values() if u.study_group == "control")
    ...
    data = {
        ...
        "data_type": (
            "SIMULATED — word-overlap proxy. Real semantic "
        ...
        "privacy": {
            ...
            "verification_method": (
                "grep 'pre_thinking' oop_alignr.py "
...
if __name__ == "__main__":
    ...
    research = run_simulation(n_feedback=120, n_control=80, sessions_per_user=10)
