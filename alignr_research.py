...
ax1.plot(
    ...
    marker="o",
    markersize=5,
    label=f"Feedback (n={N_FEEDBACK})",
    zorder=3,
)
ax1.fill_between(
    sessions, fb_mean - fb_std, fb_mean + fb_std, alpha=0.15, color="#00BFFF"
)
ax1.plot(
    sessions,
    ct_mean,
...
ax1.fill_between(
    sessions, ct_mean - ct_std, ct_mean + ct_std, alpha=0.15, color="#ff6b6b"
)
...
ax1.set_title(
    "RAS Trajectory Over 10 Sessions\n(Simulated data — replace Day 63)",
    color="#ffffff",
    fontsize=12,
    fontweight="bold",
)
ax1.legend(facecolor="#1a1a26", edgecolor="#2a2a40", labelcolor="#d0d0e0", fontsize=10)
...
plt.suptitle(
    "ALIGNR Study — Figure 1 (SIMULATED DATA)", color="#7a7a9a", fontsize=10, y=0.02
)
plt.tight_layout()
plt.savefig(
    "alignr_figure1_simulated.png", dpi=150, bbox_inches="tight", facecolor="#0a0a0f"
)
