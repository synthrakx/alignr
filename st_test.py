...
tests = [
    (
        ...
        0.60,
        "similar texts → score should be HIGH",
    ),
    (
        "I enjoy cooking pasta on Sundays",
        "Quantum computing uses qubits in superposition",
        0.10,
        "unrelated texts → score should be LOW",
    ),
    (
        "ALIGNR measures cognitive alignment",
        "ALIGNR measures cognitive alignment",
        0.98,
        "identical texts → score should be VERY HIGH",
    ),
]

all_pass = True
for t1, t2, min_score, label in tests:
    score = float(cosine_similarity(model.encode([t1]), model.encode([t2]))[0][0])
    ok = score >= min_score
    if not ok:
        all_pass = False
...
if all_pass:
    ...
    print(
        "Diagnosis: pip install sentence-transformers --upgrade --break-system-packages"
    )
